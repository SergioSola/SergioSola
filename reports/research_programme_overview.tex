...
where:
\begin{itemize}
\item The upper-right block $W_t^{\text{credit}}$ ($n_B \times n_N$): bank \$i\$'s credit risk exposure to NBFI \$j\$
\item The lower-left block $W_t^{\text{funding}}$ ($n_N \times n_B$): NBFI \$j\$'s funding risk from bank \$i\$
...
\begin{itemize}
\item A time series of adjacency matrices $W_t^{\text{credit}}$ ($n_B \times n_N$) and $W_t^{\text{funding}}$ ($n_N \times n_B$), one pair per quarter.
...
\end{itemize}
\textbf{$\rightarrow$ Link to Module 3}: The combined system matrix $\mathcal{W}_t$ is the direct input to the spectral analysis. The HLI is the largest eigenvalue of $\mathcal{W}_t$.
