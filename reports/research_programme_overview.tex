...
% Allow line breaks at underscores and slashes in \texttt
\renewcommand{\texttt}[1]{\begingroup\ttfamily\hyphenchar\font=`\-\spaceskip=.5em plus .3em minus .2em #1\endgroup}
...
{\small
\begin{longtable}{@{}|p{0.08\textwidth}|p{0.06\textwidth}|p{0.20\textwidth}|p{0.17\textwidth}|p{0.12\textwidth}|p{0.10\textwidth}|@{}}
\hline
...
{\small
\noindent\fbox{\parbox{0.17\textwidth}{\centering\textbf{Module 1}\\VaR Channel\\[2pt]\scriptsize Sub-sector $\beta_s$\\procyclicality\\+ fire-sale sim}}%
\hfill$\to$\hfill%
\fbox{\parbox{0.17\textwidth}{\centering\textbf{Module 2}\\DCC-GARCH\\[2pt]\scriptsize Time-varying\\bank--NBFI\\correlations}}%
\hfill$\to$\hfill%
\fbox{\parbox{0.17\textwidth}{\centering\textbf{Module 3}\\Quantile VAR\\[2pt]\scriptsize Tail-specific MP\\transmission\\IRFs + connect.}}%
\hfill$\to$\hfill%
\fbox{\parbox{0.17\textwidth}{\centering\textbf{Module 4}\\LS-GaR\\[2pt]\scriptsize NBFI leverage\\fattens tails\\(variance ch.)}}%
\hfill$\to$\hfill%
\fbox{\parbox{0.17\textwidth}{\centering\textbf{Module 5}\\Banerjee\\Puzzle\\[2pt]\scriptsize Within-OFI vs\\OFI-to-real\\economy split}}
}
...
\item Implemented in \texttt{estimate\_quantile\_var()} and \texttt{estimate\_\allowbreak{}quantile\_var\_grid()} in \texttt{src/\allowbreak{}econometrics/\allowbreak{}quantile\_var.py}.
...
\noindent\fbox{\small\strut Channel 1 (Dollar Funding)} \hfill
\fbox{\small\strut Channel 2 (Portfolio Flows)} \hfill
\fbox{\small\strut Channel 3 (Global Fin Cycle)}

\smallskip
\begin{center}
$\searrow$ \hspace{3em} $\downarrow$ \hspace{3em} $\swarrow$

\smallskip
\fbox{\small\strut Module 4: Cross-Country QVAR}
$\;\longrightarrow\;$
\fbox{\small\strut Module 5: GFC Amplif.\ Panel}
$\;\longrightarrow\;$
\fbox{\small\strut Module 6: Cascade Sim}
\end{center}
...
{\small
\noindent\fbox{\parbox{0.27\textwidth}{\centering\textbf{Layer 1}\\Quantile Regression\\[2pt]\scriptsize Offsetting forces:\\interaction breaks\\down in tails}}%
\hfill$\longrightarrow$\hfill%
\fbox{\parbox{0.27\textwidth}{\centering\textbf{Layer 2}\\Delta-CoVaR\\[2pt]\scriptsize Systemic risk of\\FX hedging stress\\on bond flows}}%
\hfill$\longrightarrow$\hfill%
\fbox{\parbox{0.27\textwidth}{\centering\textbf{Layer 3}\\Quantile Connectedness\\[2pt]\scriptsize Full chain dormant\\at median, active\\at $\tau{=}0.05$}}
...
\smallskip
\fbox{\parbox{0.38\textwidth}{\centering\textbf{NBFI Panel}\\[-1pt]\scriptsize Countries with higher NBFI suffer more}}%
\hspace{1em}%
\fbox{\parbox{0.38\textwidth}{\centering\textbf{Cascade Simulation}\\[-1pt]\scriptsize Multi-round equity $\to$ FX $\to$ CIP $\to$ bonds feedback loop}}
\end{center}
}
...
\textbf{Estimation details}:
\begin{itemize}
\item \texttt{nbfi\_fx\_hedging\_panel\_regression()} and \texttt{iv\_fx\_hedging\_\allowbreak{}regression()} in \texttt{src/analysis/\allowbreak{}fx\_hedging\_contagion.py}.
...
\item \textbf{Bond flows}: Reverse by $(-2.0) \times 7.5/10 = -\$1.5$B.
