...
\end{itemize}
In practice, $W_t$ is constructed as a \textbf{two-tier structure}:
\begin{enumerate}
...
\paragraph{Construction of the Adjacency Matrix $W_t$}
...
The matrix $W^{\text{credit}}$ captures only one direction --- \textbf{credit/fire-sale risk} (bank's loss from NBFI distress). There is a second, equally important direction --- \textbf{funding risk} (NBFI's loss from bank funding withdrawal):
...
The full picture requires \textbf{both matrices}: $W^{\text{credit}}$ (bank exposed to NBFI fire sale) and $W^{\text{funding}}$ (NBFI exposed to bank funding withdrawal). The combined system is a single non-symmetric matrix where:
\begin{itemize}
\item Entry \$(i,j)\$: bank \$i\$'s credit risk from NBFI \$j\$ (weighted by NBFI leverage)
\item Entry \$(j,i)\$: NBFI \$j\$'s funding risk from bank \$i\$ (weighted by bank leverage)

\end{itemize}
For the HLI spectral analysis (Module 3), we use the combined matrix. The spectral radius of the combined system captures the \textit{total amplification potential} from both credit and funding channels operating simultaneously.
...
\textbf{Implication for the adjacency matrix}: Where from-whom-to-whom data is available, $W_t$ can be disaggregated by counterparty type, enabling separate spectral analyses for credit-intermediation vs. market-intermediation risk channels.
...
\begin{itemize}
\item A time series of adjacency matrices $W_t^{\text{credit}}$ and $W_t^{\text{funding}}$ (one pair per quarter).
\item Combined (non-symmetric) matrix for spectral analysis.
...
\end{itemize}
\textbf{$\rightarrow$ Link to Module 3}: The combined adjacency matrix $W_t$ is the direct input to the spectral analysis. The HLI is the largest eigenvalue of $W_t$.
...
Consider a shock $\varepsilon$ hitting one node. Through the network, losses propagate: node \$j\$'s distress hits bank \$i\$ with weight $W_{ij}$, then \$i\$'s distress hits others with weight $W_{ki}$, and so on. After \$n\$ rounds, total losses are:

\[\text{Total loss} \propto \varepsilon + W\varepsilon + W^2\varepsilon + W^3\varepsilon + \ldots = (I - W)^{-1}\varepsilon\]

This geometric series converges if and only if $\lambda_1(W) < 1$. The \textbf{rate of amplification} is governed by $\lambda_1$:
\begin{itemize}
...
\textbf{Estimation details}:
\begin{itemize}
\item Computed via \texttt{numpy.linalg.eigvals(W\_t)} at each quarter on the combined (credit + funding) matrix.
...
\paragraph{Extension: The Global Tail Cycle}

The standard GFC captures co-movement at the \textbf{mean} --- it answers ''do returns move together on average?'' But the relevant question for systemic risk is: \textbf{do they crash together?} A market can look diversified in normal times but exhibit near-perfect correlation during stress. The GFC misses this.

A \textbf{Global Tail Cycle (GTC)} extracts the common factor from \textbf{tail co-movements} --- correlations specifically in the left tail. There are two approaches:

\textbf{(a) Tail dependence factor}: For each country pair \$(n,m)\$, compute the time-varying lower-tail dependence coefficient $\lambda_{nm,t}^L$ (using DCC-based tail dependence or exceedance correlations on a rolling window: joint probability of both being below the 5th percentile). Then extract the first principal component of the $\lambda^L$ matrix across all pairs --- this is the Global Tail Cycle.

\textbf{(b) Quantile factor model} (Ando \& Bai 2020): Estimate a factor model directly at $\tau = 0.05$, extracting the first factor from a cross-country panel of equity returns at the 5th percentile. This literally is ''the common factor in left-tail outcomes.''

The GTC is conceptually distinct from the standard GFC. The GFC could be high (assets correlated on average) while tail dependence is low (they don't crash together). Or the GFC could be moderate but the GTC very high (normal times look diversified, but stress is perfectly correlated). The GTC captures exactly the moment when diversification fails --- when VaR constraints bind simultaneously across jurisdictions and coordinated deleveraging amplifies the initial shock.

\textbf{In Module 5}, both GFC and GTC enter the panel regression to test whether NBFI penetration amplifies \textit{average} co-movement (GFC interaction) vs. \textit{tail} co-movement (GTC interaction) differently.
...
\textbf{Model specification --- OLS panel (baseline)}:
...
A single $\text{NBFI}_{c,t}$ measure (assets as \% of GDP) is too coarse --- it conflates NBFI activities with completely different systemic implications. The results differ fundamentally depending on whether NBFIs:
\begin{itemize}
\item Lend to domestic banks vs. borrow from domestic banks
\item Lend to foreigners vs. borrow from foreigners
\item Lend to or borrow from the domestic private sector

\end{itemize}
A country where NBFIs primarily borrow from domestic banks and lend to the private sector (credit intermediation model) has a different GFC transmission profile than one where NBFIs borrow from foreigners and trade derivatives with domestic banks (market intermediation model).

\textbf{Extended specification} --- replace aggregate NBFI with counterparty-decomposed measures:

\[y_{c,t} = \alpha_c + \beta_1 \cdot \text{GFC}_t + \sum_k \beta_{2,k} \cdot \text{NBFI}_{c,t}^{(k)} + \sum_k \beta_{3,k} \cdot (\text{GFC}_t \times \text{NBFI}_{c,t}^{(k)}) + \gamma X_{c,t} + \varepsilon_{c,t}\]

where \$k\$ indexes NBFI activity types:
\begin{itemize}
\item $\text{NBFI}^{\text{bank-borrow}}$: NBFI borrowing from domestic banks (repo, credit lines) $\rightarrow$ captures credit risk channel
\item $\text{NBFI}^{\text{bank-lend}}$: NBFI lending to domestic banks (deposits, bank bonds) $\rightarrow$ captures funding risk channel
\item $\text{NBFI}^{\text{foreign-borrow}}$: NBFI cross-border borrowing $\rightarrow$ captures imported funding shock channel
\item $\text{NBFI}^{\text{foreign-lend}}$: NBFI cross-border lending $\rightarrow$ captures outward transmission channel
\item $\text{NBFI}^{\text{private-lend}}$: NBFI lending to private sector $\rightarrow$ captures real economy credit channel

\end{itemize}
The $\beta_{3,k}$ coefficients reveal \textit{which type} of NBFI activity amplifies GFC transmission most. The hypothesis: $\beta_{3,\text{foreign-borrow}}$ is largest (countries whose NBFIs rely on foreign funding are most vulnerable to global shocks), while $\beta_{3,\text{private-lend}}$ may actually be \textit{stabilising} (credit intermediation provides a buffer).

\textbf{Data source}: ''From-whom-to-whom'' financial accounts --- Fed Z.1, ECB Financial Accounts, OECD institutional sector accounts.


\paragraph{Global Tail Cycle extension}

Replace the standard GFC with the Global Tail Cycle (from Module 3 extension):

\[y_{c,t} = \alpha_c + \beta_1^{\text{GFC}} \cdot \text{GFC}_t + \beta_1^{\text{GTC}} \cdot \text{GTC}_t + \beta_3^{\text{GFC}} \cdot (\text{GFC}_t \times \text{NBFI}_{c,t}) + \beta_3^{\text{GTC}} \cdot (\text{GTC}_t \times \text{NBFI}_{c,t}) + \ldots\]

If $\beta_3^{\text{GTC}} \gg \beta_3^{\text{GFC}}$: NBFI penetration amplifies \textit{tail} co-movement far more than \textit{average} co-movement --- confirming that the NBFI systemic risk channel is fundamentally a tail phenomenon.
...
\textbf{The shock}: US equity markets fall sharply (negative equity return).

\textbf{Channel 1 --- Exchange rate (Rey et al. 2024)}: When US equities fall, global risk aversion spikes. The USD appreciates (safe-haven currency). EUR/USD falls. For the pension fund:
\begin{itemize}
...
\end{itemize}
\textbf{Channel 2 --- Yield curve (Nenova et al. 2025)}: The equity shock affects monetary policy expectations:
\begin{itemize}
\item \textit{Normal times} (moderate correction): Expectations of monetary easing $\rightarrow$ yield curve \textbf{steepens} $\rightarrow$ hedged carry (US bond yield minus hedging cost) remains \textbf{attractive} $\rightarrow$ pension fund keeps hedging and buying US bonds
\item \textit{Stress} (severe crash): Flight to safety into long-term Treasuries $\rightarrow$ yield curve \textbf{flattens/inverts} $\rightarrow$ hedged carry \textbf{collapses} $\rightarrow$ the business case for holding hedged US bonds evaporates
...
\textit{Normal times} (say, a 5\% equity correction):
\begin{itemize}
\item USD appreciates modestly $\rightarrow$ hedging cost up slightly (\textbf{negative})
\item Yield curve steepens $\rightarrow$ hedged carry improves (\textbf{positive, offsetting})
\item CIP basis barely moves $\rightarrow$ manageable
\item \textbf{Net}: forces partially cancel. Self-stabilising.

\end{itemize}
\textit{Stress} (say, a 20\% crash --- March 2020 or a severe risk-off event):
\begin{itemize}
\item USD appreciates violently $\rightarrow$ huge mark-to-market losses, hedging cost surges (\textbf{negative})
\item Yield curve flattens/inverts $\rightarrow$ hedged carry disappears (\textbf{negative, reinforcing} not offsetting)
...
\hline
Quantile factor model & Global Tail Cycle extraction (Ando \& Bai 2020) & P3 \\
\hline
...
The standard Global Financial Cycle (Rey 2013) captures co-movement at the mean. A \textbf{Global Tail Cycle} captures co-movement in the left tail --- the common factor in crash risk. These are conceptually distinct objects: markets can look diversified on average but crash together. The GTC is the risk factor that sets in motion coordinated VaR breaches and feedback loops across jurisdictions. Extracted either from tail dependence matrices (rolling exceedance correlations) or quantile factor models (Ando \& Bai 2020).
